...
exports.handler = function(event, context) {
  ...
  if (event.date == undefined && event.Records == undefined) {
    // cloudWatch Trigger
    ...
    message_param = {
      ...
      execute_date: moment(messageBody.init_date, "YYYYMMDD")
        .add(parseInt(messageBody.offset) + 1, "days")
        .format("YYYYMMDD"),
      ...
      return new Promise(async (resolve, reject) => {
        ...
        for (let i = 0; i < timeline.length; i++) {
          ...
          var shop = $(tc)
            .children("img")
            .attr("src")
            .replace("http://cache.m.ui.hsmoa.com/media/hsmoa/logo/logo_", "")
            .replace(".png", "");
          ...
          if ($(lower).length > 0) {
            ...
            items.each((idx, item) => {
              let name = $(item)
                .find(".font-13")
                .text();
              let img = $($(item).find(".table-cell")[0])
                .find("img")
                .data("src");
              let price = $(item)
                .find(".font-14")
  ...
  function makeErrorReportWorkplace(message) {
    let access_token =
      "";
